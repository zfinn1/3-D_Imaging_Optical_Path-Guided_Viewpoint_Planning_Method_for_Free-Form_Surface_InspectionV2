...
% maybe=flipud(AllPts_final2);
% maybe=AllPts_final1;
% maybe=[P1;P2;P3;P4;F1;F2;F3;F4];
maybe=allSavedViews{1,1}{3,1};
...
trisurf(faces, vertices(:,1), vertices(:,2), vertices(:,3), 'FaceColor', '[0.5 0.8 0.8]', 'EdgeColor', 'none', 'FaceAlpha', 0.6);
hold on;
...
visualizeViewField(maybe(1:4,:), maybe(5:8,:),0);
...
function visualizeViewField(nearPts, farPts,m)
    hold on;
    if m==0
    ...
    patch('Faces', [1 2 3 4], 'Vertices', farPts, 'FaceColor', 'b', 'FaceAlpha', 0.4);
        ...
        patch('Faces', [1 2 3 4], 'Vertices', nearPts, 'FaceColor', 'y', 'FaceAlpha', 0.6);
       patch('Faces', [1 2 3 4], 'Vertices', farPts, 'FaceColor', 'y', 'FaceAlpha', 0.6);
    end
    for i = 1:4
