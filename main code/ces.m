...
sign1=sign;
freq_dict1=freq_dict;

for i=1:256
    
min_value1=M*N;
 for t = 1:256 
     if (freq_dict(t)>0)&&(freq_dict(t)<min_value1)
            min_value1= freq_dict(t);  
            min_index = t; 
     end  
end  
t = min_index;
if sign(t,1)==0
    ...
    freq_dict(t)=freq_dict(t)+min_value1;
  ...
  min_value2=M*N;
  for t1=1:256
        if (freq_dict1(t1)<min_value2)&&(freq_dict1(t1)>0)
            min_value2=freq_dict1(t1);
        end
    end
    %遍历出频率矩阵的最小值
    t1=1;
    while (freq_dict1(t)~=min_value2)&&(t<256)
        t1=t1+1;
    end 
    
    if sign(t1,1)==0
        dictionary1{t1}='0';%如果该最小频率并没有子节点，就在huffman字典中编一次‘0’
    else
        dictionary1{t1}=['0',dictionary1{t1}];
        %若该最小频率的灰度值有子节点，说明他不是一个孤立的点，至少是个二元二叉树，
        %而且这个灰度点是他的左树和子树合成概率的树，所以为该树加上'0'，即为右节点加上'0'
        i=1;
        while (sign1(t1,i)~=0)&&(i<256)
            dictionary1{sign1(t1,i)}=['0',dictionary1{sign1(t1,i)}];
            %找该灰度级的子节点（或是左节点），若不为0则一直有子节点就在每一个子节点的编码前加0，直到找不到子节点
            i=i+1;
        end
    end
    freq_dict1(t1)=0;%将该灰度级的频率清0，不影响下次的编码（找最小值的时候）
    zuo1=t1;%将最小值的灰度级保存起来，方便后面嫁接入更高级的父节点的目录中
    
    secmin_value1=M*N;
    for t1=1:256
        if (freq_dict(t1)<secmin_value1)&&(freq_dict(t1)>0)
            secmin_value1=freq_dict1(t1);
        end
    end
    %遍历出频率矩阵的次最小值
    t1=1;
    while (freq_dict1(t1)~=secmin_value1)&&(t1<256)
        t1=t1+1;
    end
    %找到像素灰度频率次最小值，并标记好他的灰度值t
    if sign1(t1,1)==0
        dictionary1{t1}='1';%如果该最小频率并没有子节点，说明他就是单独一个点，就在huffman字典中编一次'1'
    else
        dictionary1{t1}=['1',dictionary1{t1}];
        %若该最小频率的灰度值有子节点，说明他不是一个孤立的点，至少是个二元二叉树，
        %而且这个灰度点是他的左树和子树合成概率的树，所以为该树加上'1'，即为右节点加上'1'
        i=1;
        while (sign1(t1,i)~=0)&&(i<256)
            dictionary1{sign1(t1,i)}=['1',dictionary1{sign1(t1,i)}];
            %找该灰度级的子节点（或是左节点），若不为0则一直有子节点就在每一个子节点的编码前加1，直到找不到子节点
            i=i+1;
        end
    end
    freq_dict1(t1)=freq_dict1(t1)+min_value2;
    %将左边更小概率灰度值的概率与右边次小概率灰度合成到右边次小灰度里
    %这样就是右节点又可视作该两个节点的父节点，左节点就是该父节点的字节点
    you1=t;%将右边的节点的灰度值保存起来，方便后面加他目录底下子节点
    i=1;
    while (sign1(t1,i)~=0)&&(i<256)
        i=i+1;
    end
    ...
    i=i+1;%在刚刚找的位置下，为新来的左节点的子节点（或是左节点）找新的位置
    j=1;
    while (sign1(zuo1,j)~=0)&&(j<61)
        sign1(t1,i)=sign1(zuo1,j);
        i=i+1;
        j=j+1;
        %把子节点的子节点放到父母结点的子节点目录下，相当于户主是爷爷，新生的孙子上到爷爷为户主的户口
        %并且该过程是节节推进的，并不会改变原有的节点放入的顺序
