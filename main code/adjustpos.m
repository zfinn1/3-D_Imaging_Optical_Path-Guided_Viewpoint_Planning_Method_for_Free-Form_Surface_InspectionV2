function [AllPts_new,P_new,F_new]=adjustpos(AllPts,Intersections,stl_file, numSamples, step_size, max_rotation)

if size(Intersections,1) < 2
    warning('交点数量不足，跳过位置调整。');
    AllPts_new = AllPts;
    P_new=AllPts(1:4,:);
    F_new=AllPts(5:8,:);
    return;
...
offsets = Intersections - rotationCenter; 

% 2. 对偏移进行PCA
[coeff, ~, ~] = pca(offsets);
% coeff(:,1) 是主要方向（最大方差方向）

% 3. 判断主要方向与视场向量（例如宽向量）的夹角
angle_with_wide = acosd( dot(coeff(:,1), wide_vec) / (norm(coeff(:,1)) * norm(wide_vec)) );

% 4. 根据夹角决定旋转轴与旋转方向
if angle_with_wide < 90
    % 如果主要偏离方向与宽向量较为一致，则可能需要绕长向量旋转
    axis_vec = long_vec;
    % 旋转方向可根据偏移向量的具体分布确定
    rotation_direction = sign( dot(coeff(:,1), some_reference_direction1) ); 
else
    % 否则，绕宽向量旋转
    axis_vec = wide_vec;
    rotation_direction = sign( dot(coeff(:,1), some_reference_direction2) );
end
%  [AllPts_new,P_new, F_new] = applyRotation(AllPts, 5,rotationCenter,-wide_vec);
 [AllPts_new, P_new, F_new] = autoRotateViewfield(AllPts, stl_file, numSamples, step_size, max_rotation,rotationCenter,rotation_direction,axis_vec);

% [AllPts_new,P_new, F_new] = applyRotation(AllPts_new, 3.1,rotationCenter,-long_vec);
end
