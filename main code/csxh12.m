...
%% 参数设置
numRings = 2;           % 需要生成的视场圈数（包括第一圈）
numViewsRing = 13; % 第一圈视场数量

% 保存各圈视场的 cell 数组，每个元素为一个视场的 8x3 顶点矩阵
allRings.Views = cell(1, numRings);  % 存储每圈的视场数据
allRings.Intersections = cell(1, numRings);  % 存储每圈的交点信息

for ring = 1:numRings
    % 调用rotatefullcircle生成当前环的视场数据、交点信息以及新的点数据
    [currentSavedViews, currentIntersections] = rotatefullcircle(AllPts_prev, stl_file, n_ref, numViewsRing);
    
    % 可视化当前环的视场
    visualize(currentSavedViews);
    
    %更新AllPts_prev
    AllPts_last = currentSavedViews{1};
    [AllPts_prev, P_new,~] = autoRotateViewfield(AllPts_last, stl_file, 0.1, 15, 1, 360, 0.8,3);
    [n_ref, ~] = computePlane(P_new(1,:),P_new(2,:),P_new(3,:));
     n_ref=n_ref/norm(n_ref);
    
    % 将当前环的数据保存到对应的cell数组中
    allRings.Views{ring} = currentSavedViews;
    allRings.Intersections{ring} = currentIntersections;
