...
function maybe_new=adjustVdthroughN1(maybe_new,faces,vertices)
...
ptsP = sampleFace(P_new, 40);
ptsF = sampleFace(F_new, 40);
% 利用 inpolyhedron 判断采样点是否在模型内部（返回 true 表示在内部）
 ...
 % 计算内部点的比例
    ratioInsideP = sum(insideP) / 1600;  % 近面内部点占比
    ratioInsideF = sum(insideF) / 1600;  % 远面内部点占比
disp( ratioInsideP);
disp( ratioInsideF);
end
