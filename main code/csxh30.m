...
% target_z=allSavedViews{1,1}{1,1};
target_z=50;
...
%    if circle==5 || circle==4
%    previousViewField = prevViews{1};
%    end
    % 自动调整视场域：旋转一定角度直到触发相切条件
    [AllPts_prev, P_new, ~] = autoRotateViewfield(previousViewField, stl_file, 0.1, 15, 1, 360, 0.8, 2);
    ...
    [savedViews_current, savedIntersections_current, Q] = rotatefullcircle1(AllPts_prev, stl_file, current_n_ref, 15);
    
    % 保存当前圈的结果
    allSavedViews{circle} = savedViews_current;
    allSavedIntersections{circle} = savedIntersections_current;
    
%    allSavedViews{circle,1}{1,1} =AllPts_prev ;
...
%            visualizeNormalVector1(viewField);
        end
    end
end
axis equal;
