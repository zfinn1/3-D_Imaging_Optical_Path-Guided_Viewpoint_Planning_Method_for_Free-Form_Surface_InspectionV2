...
savedViews = cell(numSteps+1, 1);      % 保存初始视场和每一步旋转后的视场
savedIntersections = cell(numSteps+1, 1);% 用于保存直接旋转模式下的交点（如果有的话）
...
for step = 1:numSteps
    ...
    if step==3
          rotationCenter1 = (currentViewField(1,:) + currentViewField(2,:)+currentViewField(5,:) + currentViewField(6,:)) / 4;
             axis_vec1 = (currentViewField(1,:) - currentViewField(2,:)) / norm(currentViewField(1,:) -currentViewField(2,:));
             AllPts_new = rotatePoints(currentViewField, rotationCenter1, axis_vec1, 2);
...
newCuboid = autoRotatebasefield(basefield, stl_file, 0.2, numSamples, 1, max_rotation, threshold_param);
...
%         if step==1
        [AllPts_new, P_new, F_new] = autoRotateViewfield(currentViewField, stl_file, epsilon, numSamples, step_size, max_rotation, 0.8,3);
%         else
%              rotationCenter = (P_new(3,:) + P_new(2,:)+F_new(3,:) + F_new(2,:)) / 4;
%              axis_vec = (P_new(3,:) - P_new(2,:)) / norm(P_new(3,:) - P_new(2,:));
%              AllPts_new = rotatePoints(currentViewField, rotationCenter, axis_vec, 173.5);
