...
green_matrix=f0;%对图像的像素进行调整
[M,N]=size(green_matrix);

freq_dict = zeros(1, 256); % 初始化一个大小为256的数组来存储频率  
for i = 1:size(green_matrix, 1)  
    for j = 1:size(green_matrix, 2)  
        pixel_value = green_matrix(i, j);  
...
%     t = min_index;
    if sign(t,1)==0
        dictionary1{t}='0';%如果该最小频率并没有子节点，就在huffman字典中编一次‘0’
    else
        dictionary1{t}=['0',dictionary1{t}];
        %若该最小频率的灰度值有子节点，说明他不是一个孤立的点，至少是个二元二叉树，
        %而且这个灰度点是他的左树和子树合成概率的树，所以为该树加上'0'，即为右节点加上'0'
        i=1;
        while (sign(t,i)~=0)&&(i<256)
            dictionary1{sign(t,i)}=['0',dictionary1{sign(t,i)}];
...
%     t = min_index;
    ...
    if sign(t,1)==0
        dictionary1{t}='1';%如果该最小频率并没有子节点，说明他就是单独一个点，就在huffman字典中编一次'1'
    else
        dictionary1{t}=['1',dictionary1{t}];
        %若该最小频率的灰度值有子节点，说明他不是一个孤立的点，至少是个二元二叉树，
        %而且这个灰度点是他的左树和子树合成概率的树，所以为该树加上'1'，即为右节点加上'1'
        i=1;
        while (sign(t,i)~=0)&&(i<256)
            dictionary1{sign(t,i)}=['1',dictionary1{sign(t,i)}];
...
fc = cell(M, N);
k= green_matrix < 256;
fc(k) = dictionary1(green_matrix(k) + 1);
...
imcore1 = sprintf('%s', fc2{:});

save('green.mat','imcore1','dictionary1');