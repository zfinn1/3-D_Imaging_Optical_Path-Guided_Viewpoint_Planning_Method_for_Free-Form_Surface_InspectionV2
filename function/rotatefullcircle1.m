function [savedViews,savedIntersections,Q]=rotatefullcircle1(AllPts_prev,stl_file,n_ref,numViewsFirstRing) 
Q=[];
...
%            normal_prev = getModelNormalAt(mean(AllPts_prev(1:4,:), 1), model);
          [normal_prev, ~] = computePlane(currentViewField(1,:), currentViewField(2,:), currentViewField(3,:));
           ...
           if dot(normal_prev,n_ref)>0
            ...
            if max_angle_to_ref > 45
                fprintf('法向量偏差较大，调整 autoRotatebasefield 参数。\n');
                threshold_param = 0.6;  % 调整 autoRotatebasefield 的 threshold 参数
            else
                threshold_param = 0.9;  % 正常参数
            end
            [newSquareVertices, basefield] = generateSquareFromProjection(P_new, mf, 10);
            newCuboid = autoRotatebasefield(basefield, stl_file, 0.1, numSamples, 1, 360, threshold_param);
            P_new = newCuboid(1:4,:);
