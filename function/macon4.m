...
% isIntersections = computeIntersectionsWithKD(AllPts(1:4,:), model, 1e-2, kdtree1, 8);
% 
% n_plane=getModelNormalAt(P(4,:), model);
% n_plane=n_plane/norm(n_plane);
%     intersections = [];
%    for i = 1:size(isIntersections,1)
%         pt = isIntersections(i,:);
% %         scatter3(pt(1),pt(2),pt(3), 80, 'r', 'filled');
...
%    end
%    
%   % 假设 intersections 是你的交点集合，允许 z 坐标偏差为 1 单位
% intersections = filterIntersectionsByZ(intersections, 1);
%  
% scatter3(intersections(2,1),intersections(2,2),intersections(2,3), 80, 'r', 'filled');
% [angle_deg, mf, mn] = computeAngleFromIntersectionsAndEdge(P(3,:), P(4,:), intersections);
% 
% 
% scatter3(mf(1),mf(2),mf(3), 80, 'k', 'filled');
% scatter3(mn(1),mn(2),mn(3), 80, 'k', 'filled');
% %  scatter3(intersections(7,1),intersections(7,2),intersections(7,3), 80, 'r', 'filled');
% [AllPts_new,P_new,F_new]=adjustpos(AllPts,isIntersections,stl_file, 15, 1, 360);
% isIntersections1 = computeIntersectionsWithKD(AllPts_new(1:4,:), model, 1e-2, kdtree1, 8);
% [AllPts_new,P_new,F_new]=adjustpos(AllPts_new,isIntersections1,stl_file, 15, 1, 360);
% isIntersections2 = computeIntersectionsWithKD(AllPts_new(1:4,:), model, 1e-2, kdtree1, 8);
% [AllPts_new,P_new,F_new]=adjustpos(AllPts_new,isIntersections2,stl_file, 15, 1, 360);
% isIntersections3 = computeIntersectionsWithKD(AllPts_new(1:4,:), model, 1e-2, kdtree1, 8);
% [AllPts_new,P_new,F_new]=adjustpos(AllPts_new,isIntersections3,stl_file, 15, 1, 360);
%  [orderedNear,orderedFar]=orderPts(AllPts,AllPts_new);
% maybe222= [orderedNear;orderedFar]; 
% maybe333=adjustVdthroughN(maybe222,faces,vertices);
% maybe333=adjustVdthroughN(maybe333,faces,vertices);0
% 
...
% 
% subplot(1,2,2)
% visualizeViewField(maybe333(1:4,:), maybe333(5:8,:));
%  trisurf(faces1, vertices1(:,1), vertices1(:,2), vertices1(:,3), ...
%         'FaceColor', 'cyan', 'EdgeColor', 'k', 'FaceAlpha', 0.1);
%     hold on;
% %  visualizeNormalVector(E_new_new, 'm');
% % 统一设置
