...
new_pv = adjustViewpoint(center , view_dir, faces, vertices, 1);
center=new_pv;
% 投影平面上构建矩形的四个角点
half_w = width / 2;
half_h = height / 2;
...
function new_pv = adjustViewpoint(pv, normal, faces, vertices, min_offset)
    offset = min_offset;
    new_pv = pv;
    while in_polyhedron(faces, vertices, new_pv)  % 或检测视场盒子中心
        new_pv = pv + offset * normal;
        offset = offset + 1; % 逐步增加
    end
end