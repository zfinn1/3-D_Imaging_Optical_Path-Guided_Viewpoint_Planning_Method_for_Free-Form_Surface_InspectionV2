function [savedViews,savedIntersections] = rotatefullcircle9(AllPts_prev, stl_file, n_ref, numViewsFirstRing,kdtree)
 ...
 currentViewField= [orderedNear;orderedFar];
  ...
  disp(intersections);
        ...
        if isempty(intersections)
            ...
            [AllPts_new, P_new, F_new] = autoRotateViewfield(currentViewField, stl_file, 0.2, numSamples, step_size, max_rotation, 0.85,0.1 ,1);
...
%             [newCuboid, ~, ~] = autoRotateViewfield(basefield, stl_file, 0.1, numSamples, 1, max_rotation, threshold_param,0.1,4);
                ...
                if dot(normal_prev, n_ref) > 0
                    ...
                    [AllPts_new, P_new, F_new] = autoRotateViewfield(currentViewField, stl_file, 0.2, numSamples, step_size, max_rotation, 0.3,0.05 ,1);
                    currentViewField = AllPts_new;
        ...
        [AllPts_new, P_new, F_new] = autoRotateViewfield(currentViewField, stl_file, 0.2, numSamples, 1, max_rotation, 0.85,0.1, 1);
        currentViewField = AllPts_new;
    end

    % 视场保存
    disp(currentViewField);
