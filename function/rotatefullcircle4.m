function [savedViews,savedIntersections,Q]=rotatefullcircle4(AllPts_prev,stl_file,n_ref,numViewsFirstRing,G)
Q=[];
...
%            normal_prev = getModelNormalAt(mean(AllPts_prev(1:4,:), 1), model);
          [normal_prev, ~] = computePlane(currentViewField(1,:), currentViewField(2,:), currentViewField(3,:));
           ...
           if dot(normal_prev,n_ref)>0
               fprintf('夹角过大且法向量在同侧采用直接旋转模式。\n');
               [AllPts_new, P_new, F_new] = autoRotateViewfield1(currentViewField, stl_file, epsilon, numSamples, step_size, max_rotation, 0.3,1);
                
                currentViewField = AllPts_new;
             %保存交点 
                savedIntersections{step+1} = intersections;
           else
             fprintf('夹角过大且法向量在不同侧采用基准重构后旋转模式。\n');
            % 同样调用基准重构后旋转模式
            intersection_normals = [];
            for i = 1:size(intersections,1)
                pt = intersections(i,:);
            ...
            [newSquareVertices, basefield] = generateSquareFromProjection(P_new, mf, 10);
            newCuboid = autoRotatebasefield(basefield, stl_file, 0.1, numSamples, 1, 360, 0.9);
            P_new = newCuboid(1:4,:);
            F_new = newCuboid(5:8,:);
            currentViewField = newCuboid;
           end
    ...
    %保存每个视场
    if previntersectionflag==0
       savedViews{step+1} = currentViewField;
    else
       savedViews{step+2} = currentViewField; 
    end
    if checkOBBIntersection(AllPts_prev,currentViewField,0.11) ||checkOBBIntersection(G,currentViewField,0.11)
        if step==1
            savedViews{1} = currentViewField;
            disp('循环视场第一步就出现出现重叠，动态调整');
        else
