function [savedViews,savedIntersections]=rotatefullcircle(AllPts_prev,stl_file,n_ref,numViewsFirstRing)
...
for step = 1:numViewsFirstRing
    ...
    % 过滤交点：根据Z方向过滤（你已有 filterIntersectionsByZ 函数）
    ...
    if checkOBBIntersection(AllPts_prev, currentViewField, 0.11)
        if step == 1
            % 如果第一步就出现重叠，进行动态调整：
            disp('循环视场第一步出现重叠，动态调整中...');
            % 如果调整后仍存在交点，则采用直接旋转模式并保存交点
            if ~isempty(intersections)
                fprintf('动态调整后仍检测到交点，采用直接旋转模式并储存交点。\n');
                [AllPts_new, P_new, F_new] = autoRotateViewfield1(currentViewField, stl_file, epsilon, numSamples, step_size, max_rotation, 0.3, 1);
                currentViewField = AllPts_new;
                savedIntersections{step+1} = intersections;
            else
